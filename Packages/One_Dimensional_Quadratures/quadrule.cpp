...
//
//    Input, long double ALPHA, the value of Alpha, if needed.
//
//    Input, long double BETA, the value of Beta, if needed.
//
//    Output, long double T[NT], the knots.
//
//    Output, long double WTS[NT], the weights.
//
{
  long double *aj;
  long double *bj;
...
//
  aj = new long double[nt];
  bj = new long double[nt];
...
//    Input, long double BETA, the value of Beta, if needed.
//
//    Input, long double A, B, the interval endpoints, or
//    other parameters.
//
...
//
//    Output, long double X[N], the abscissas.
//
//    Output, long double W[N], the weights.
//
{
  int i;
...
void chebyshev1_compute ( int n, long double x[], long double w[] )
...
{
  int i;
  const long double r8_pi = 3.141592653589793;
  ...
  for ( i = 0; i < n; i++ )
  {
    w[i] = r8_pi / ( long double ) ( n );
  }
  for ( i = 0; i < n; i++ )
  {
    x[i] = cos ( r8_pi * ( long double ) ( 2 * n - 1 - 2 * i ) 
                       / ( long double ) ( 2 * n ) );
  }
...
//
//    Output, long double CHEBYSHEV1_INTEGRAL, the value of the exact integral.
//
{
  long double bot;
  long double exact;
...
//
  if ( ( expon % 2 ) == 0 )
  {
    ...
    exact = r8_pi * ( long double ) ( top ) / ( long double ) ( bot );
  }
  else
  {
    exact = 0.0;	
...
void chebyshev2_compute ( int n, long double x[], long double w[] )
...
//
{
  ...
  for ( i = 0; i < n; i++ )
  {
    angle = r8_pi * ( long double ) ( n - i ) / ( long double ) ( n + 1 );
    w[i] = r8_pi / ( long double ) ( n + 1 ) * pow ( sin ( angle ), 2 );
    x[i] = cos ( angle );
  }
...
//    Input, int EXPON, the exponent.
//
//    Output, long double CHEBYSHEV2_INTEGRAL, the value of the exact integral.
//
{
  long double bot;
  long double exact;
...
void chebyshev3_compute ( int n, long double x[], long double w[] )
...
//
//    If the order is long doubled, the abscissas of the new rule include
...
//
  ...
  for ( i = 0; i < n; i++ )
  {
    angle = ( long double ) ( n - 1 - i ) * r8_pi / ( long double ) ( n - 1 );
    x[i] = cos ( angle );
  }

  w[0] = r8_pi / ( long double ) ( 2 * ( n - 1 ) );
  for ( i = 1; i < n - 1; i++ )
  {
    w[i] = r8_pi / ( long double ) ( n - 1 );
  }
  w[n-1] = r8_pi / ( long double ) ( 2 * ( n - 1 ) );
...
//    Input, int EXPON, the exponent.
//
//    Output, long double CHEBYSHEV3_INTEGRAL, the value of the exact integral.
//
{
  long double bot;
  long double exact;
...
//    Input, long double BETA, the value of Beta, if needed.
//
//    Output, long double AJ[M], BJ[M], the diagonal and subdiagonal
//    of the Jacobi matrix.
//
//    Output, long double CLASS_MATRIX, the zero-th moment.
//
{
  long double a2b2;
  long double ab;
...
//
{
  ...
  theta = new long double[n];

  for ( i = 1; i <= n; i++ )
  {
    theta[i-1] = ( long double ) ( i - 1 ) * r8_pi 
               / ( long double ) ( n - 1 );
  }
  ...
  for ( i = 0; i < n; i++ )
  {
    ...
    for ( j = 1; j <= ( ( n - 1 ) / 2 ); j++ )
    {
      ...
      w[i] = w[i] - b * cos ( 2.0 * ( long double ) ( j ) * theta[i] ) 
           / ( long double ) ( 4 * j * j - 1 );
    }
  }

  w[0] = w[0] / ( long double ) ( n - 1 );
  for ( i = 2; i <= n-1; i++ )
  {
    w[i-1] = 2.0 * w[i-1] / ( long double ) ( n - 1 );
  }
  w[n-1] = w[n-1] / ( long double ) ( n - 1 );
...
void clenshaw_curtis_set ( int order, long double x[], long double w[] )
...
//
//    Output, long double X[ORDER], the abscissas.
//
//    Output, long double W[ORDER], the weights.
//
{
  if ( order == 1 )
  {
...
//
{
  ...
  for ( i = 1; i <= n; i++ )
  {
    theta[i-1] = ( long double ) ( 2 * ( n - i ) + 1 ) * r8_pi
               / ( long double ) ( 2 * n     );
  }
...
//    between 1 and 10.
//
//    Output, long double XTAB[N], the abscissas.
//
//    Output, long double WEIGHT[N], the weights.
//
{
  if ( n == 1 )
  {
...
//
{
  ...
  for ( i = 1; i <= n; i++ )
  {
    theta[i-1] = ( long double ) ( n + 1 - i ) * r8_pi
               / ( long double ) ( n + 1     );
  }
  ...
  for ( i = 0; i < n; i++ )
  {
    ...
    for ( j = 1; j <= ( ( n - 1 ) / 2 ); j++ )
    {
      w[i] = w[i] - 2.0 * cos ( 2.0 * ( long double ) ( j ) * theta[i] ) 
        / ( long double ) ( 4 * j * j - 1 );
    }

    if ( 2 < n )
    {
      p = 2.0 * ( long double ) ( ( n + 1 ) / 2 ) - 1.0;
...
//    Input, int EXPON, the exponent.
//
//    Input, long double ALPHA, the exponent of (1-X^2) in the weight factor.
//
//    Output, long double GEGENBAUER_INTEGRAL, the value of the integral.
//
{
  long double arg1;
  long double arg2;
...
//
//    Input, long double ALPHA, the exponent of (1-X^2) in the weight.  
//    -1.0 < ALPHA is required.
//
//    Input, long double A, B, the left and right endpoints 
//    of the interval.
//
//    Output, long double X[N], the abscissas.
//
...
void gegenbauer_ss_compute ( int order, long double alpha, long double xtab[], 
  long double weight[] )
...
//    -1.0 < ALPHA is required.
//
//    Output, long double XTAB[ORDER], the abscissas.
//
//    Output, long double WEIGHT[ORDER], the weights.
//
{
  long double an;
  long double *c;
...
//
  ...
  for ( i = 3; i <= order; i++ )
  {
    c[i-1] = ( long double ) ( i - 1 ) 
          * ( alpha + alpha + ( long double ) ( i - 1 ) ) / 
          ( ( alpha + alpha + ( long double ) ( 2 * i - 1 ) ) 
          * ( alpha + alpha + ( long double ) ( 2 * i - 3 ) ) );
  }
  ...
  for ( i = 1; i <= order; i++ )
  {
    if ( i == 1 )
    {
      an = alpha / ( long double ) ( order );

      r1 = ( 1.0 + alpha ) 
        * ( 2.78 / ( 4.0 + ( long double ) ( order * order ) ) 
        + 0.768 * an / ( long double ) ( order ) );
    ...
    else if ( i == 2 )
    {
      ...
      r2 = 1.0 + 0.06 * ( ( long double ) ( order ) - 8.0 ) * 
        ( 1.0 + 0.12 * alpha ) / ( long double ) ( order );

      r3 = 1.0 + 0.012 * alpha * 
        ( 1.0 + 0.25 * fabs ( alpha ) ) / ( long double ) ( order );
    ...
    else if ( i == 3 )
    {
      ...
      r2 = 1.0 + 0.22 * ( ( long double ) ( order ) - 8.0 ) 
        / ( long double ) ( order );

      r3 = 1.0 + 8.0 * alpha / 
        ( ( 6.28 + alpha ) * ( long double ) ( order * order ) );
    ...
    else if ( i == order - 1 )
    {
      ...
      r2 = 1.0 / ( 1.0 + 0.639 
        * ( ( long double ) ( order ) - 4.0 ) 
        / ( 1.0 + 0.71 * ( ( long double ) ( order ) - 4.0 ) ) );

      r3 = 1.0 / ( 1.0 + 20.0 * alpha / ( ( 7.5 + alpha ) * 
        ( long double ) ( order * order ) ) );
    ...
    else if ( i == order )
    {
      ...
      r2 = 1.0 / 
        ( 1.0 + 0.22 * ( ( long double ) ( order ) - 8.0 ) 
        / ( long double ) ( order ) );
...
//  Parameters:
//
//    Output, long double &P2, the value of J(ORDER)(X).
//
//    Output, long double &DP2, the value of J'(ORDER)(X).
//
//    Output, long double &P1, the value of J(ORDER-1)(X).
//
//    Input, long double X, the point at which polynomials are evaluated.
//
//    Input, int ORDER, the order of the polynomial to be computed.
//
//    Input, long double ALPHA, the exponents of (1-X^2).
//
//    Input, long double C[ORDER], the recursion coefficients.
//
{
  long double dp0;
  long double dp1;
...
//  Parameters:
//
//    Input/output, long double &X, the approximate root, which
...
//    Input, int ORDER, the order.
//
//    Input, long double ALPHA, the parameter.
//    -1.0 < ALPHA.
//
//    Output, long double X[ORDER], the abscissas.
//
...
//
{
  long double alpha_laguerre;
  long double arg;
  int i;
  int order_laguerre;
  long double *w_laguerre;
  long double *x_laguerre;
  ...
  w_laguerre = new long double[order_laguerre];
  x_laguerre = new long double[order_laguerre];
...
void gen_hermite_ek_compute ( int n, long double alpha, long double x[], long double w[] )
...
//
  bj = new long double[n];

  for ( i = 0; i < n; i++ )
  {
    i_r8 = ( long double ) ( i + 1 );
    if ( ( i % 2 ) == 0 )
    {
      bj[i] = ( i_r8 + alpha ) / 2.0;
    }
...
//    0 <= EXPON.
//
//    Input, long double ALPHA, the exponent of |X| in the weight function.
//    -1.0 < ALPHA.
//
//    Output, long double GEN_HERMITE_INTEGRAL, the value of the integral.
//
{
  long double a;
  long double arg;
...
void gen_laguerre_ek_compute ( int n, long double alpha, long double x[], long double w[] )
...
//    Input, int N, the order.
//
//    Input, long double ALPHA, the exponent of the X factor.
...
//    -1.0 < ALPHA.
//
//    Output, long double GEN_LAGUERRE_INTEGRAL, the value of the integral.
//
{
  long double arg;
  long double value;

  arg = alpha + ( long double ) ( expon + 1.0 );
  value = tgamma ( arg );
...
//
  for ( i = 0; i < order; i++ )
  {
    b[i] = ( alpha + ( long double ) ( 2 * i + 1 ) );
  }

  for ( i = 0; i < order; i++ )
  {
    c[i] = ( long double ) ( i ) * ( alpha + ( long double ) ( i ) );
  }
  prod = 1.0;
  for ( i = 1; i < order; i++ )
  {
...
//
    if ( i == 0 )
    {
      xval = ( 1.0 + alpha ) * ( 3.0+ 0.92 * alpha ) / 
        ( 1.0 + 2.4 * ( long double ) ( order ) + 1.8 * alpha );
    }
    else if ( i == 1 )
    {
      xval = xval + ( 15.0 + 6.25 * alpha ) / 
        ( 1.0 + 0.9 * alpha + 2.5 * ( long double ) ( order ) );
    }
    else
    {
      r1 = ( 1.0 + 2.55 * ( long double ) ( i - 1 ) ) 
        / ( 1.9 * ( long double ) ( i - 1 ) );

      r2 = 1.26 * ( long double ) ( i - 1 ) * alpha / 
...
//  Parameters:
//
//    Output, long double *P2, the value of L(ORDER)(X).
//
//    Output, long double *DP2, the value of L'(ORDER)(X).
//
//    Output, long double *P1, the value of L(ORDER-1)(X).
//
//    Input, long double X, the point at which polynomials are evaluated.
//
//    Input, int ORDER, the order of the polynomial to be computed.
//
//    Input, long double ALPHA, the exponent of the X factor in the
//    integrand.
//
//    Input, long double B[ORDER], C[ORDER], the recursion coefficients.
//
{
  long double dp0;
  long double dp1;
...
//    0 <= N.
//
//    Output, long double VALUE, the value of the integral.
//
{
  const long double r8_pi = 3.141592653589793;
  long double value;
...
void hermite_set ( int n, long double x[], long double w[] )
...
void hermite_1_set ( int n, long double x[], long double w[] )
...
//
{
  ...
  cc = sqrt ( r8_pi ) * tgamma ( ( long double ) ( order ) ) 
    / pow ( 2.0, order - 1 );

  s = pow ( 2.0 * ( long double ) ( order ) + 1.0, 1.0 / 6.0 );
  ...
  for ( i = 0; i < ( order + 1 ) / 2; i++ )
  {
    ...
    {
      x = x - 1.14 * pow ( ( long double ) ( order ), 0.426 ) / x;
    }
    else if ( i == 2 )
    {
...
void hermite_ss_recur ( long double *p2, long double *dp2, long double *p1, long double x, int order )
...
//  Parameters:
//
//    Output, long double *P2, the value of H(ORDER)(X).
//
//    Output, long double *DP2, the value of H'(ORDER)(X).
//
//    Output, long double *P1, the value of H(ORDER-1)(X).
//
//    Input, long double X, the point at which polynomials are evaluated.
//
//    Input, int ORDER, the order of the polynomial to be computed.
//
{
  ...
  for ( i = 2; i <= order; i++ )
  {
    ...
    q2  = x * q1 - 0.5 * ( ( long double ) ( i ) - 1.0 ) * q0;
    dq2 = x * dq1 + q1 - 0.5 * ( ( long double ) ( i ) - 1.0 ) * dq0;
  }
...
//  Purpose:
//
//    I4_FACTORIAL2 computes the long double factorial function.
//
//  Discussion:
//
...
//  Parameters:
//
//    Input, int N, the argument of the long double factorial function.
//    If N is less than 1, I4_FACTORIAL2 is returned as 1.
//
//    Output, int I4_FACTORIAL2, the value of the long double factorial of N.
//
{
  int value;
...
//
//    Input/output, long double D(N), the diagonal entries of the matrix.
//    On output, the information in D has been overwritten.
//
//    Input/output, long double E(N), the subdiagonal entries of the 
//    matrix, in entries E(1) through E(N-1).  On output, the information in
//    E has been overwritten.
//
//    Input/output, long double Z(N).  On input, a vector.  On output,
...
void jacobi_ek_compute ( int n, long double alpha, long double beta, long double x[], 
  long double w[] )
...
//    Input, int N, the order.
//
//    Input, long double ALPHA, BETA, the exponents of (1-X) and
...
//    Input, int EXPON, the exponent.
//
//    Input, long double ALPHA, the exponent of (1-X) in the weight factor.
//
//    Input, long double BETA, the exponent of (1+X) in the weight factor.
//
//    Output, long double JACOBI_INTEGRAL, the value of the integral.
//
{
  long double arg1;
  long double arg2;
...
//
  for ( i = 1; i <= order; i++ )
  {
    ...
    else
    {
      b[i-1] = ( alpha + beta ) * ( beta - alpha ) / 
             ( ( alpha + beta + ( long double ) ( 2 * i ) ) 
             * ( alpha + beta + ( long double ) ( 2 * i - 2 ) ) );
    }
    ...
    {
      c[i-1] = 4.0 * ( long double ) ( i - 1 ) 
         * ( alpha + ( long double ) ( i - 1 ) ) 
          * ( beta + ( long double ) ( i - 1 ) ) 
            * ( alpha + beta + ( long double ) ( i - 1 ) ) / 
            ( ( alpha + beta + ( long double ) ( 2 * i - 1 ) ) 
            * pow ( alpha + beta + ( long double ) ( 2 * i - 2 ), 2 ) 
            * ( alpha + beta + ( long double ) ( 2 * i - 3 ) ) );
    }
  }
  ...
  for ( i = 1; i <= order; i++ )
  {
    ...
    else if ( i == 3 )
    {
      ...
      r3 = 1.0 + 8.0 * beta / 
        ( ( 6.28 + beta ) * ( long double ) ( order * order ) );
...
void laguerre_ek_compute ( int n, long double x[], long double w[] )
...
//
  ...
  for ( i = 0; i < n; i++ )
  {
    bj[i] = ( long double ) ( i + 1 );
  }

  for ( i = 0; i < n; i++ )
  {
    x[i] = ( long double ) ( 2 * i + 1 );
  }
...
//    0 <= EXPON.
//
//    Output, long double EXACT, the value of the integral.
//
{
  long double exact;
...
void laguerre_set ( int n, long double x[], long double w[] )
...
void laguerre_1_set ( int n, long double x[], long double w[] )
...
//
    if ( i == 0 )
    {
      x =  3.0 / ( 1.0 + 2.4 * ( long double ) ( order ) );
    }
    else if ( i == 1 )
    {
      x = x + 15.0 / ( 1.0 + 2.5 * ( long double ) ( order ) );
    }
    else
    {
      r1 = ( 1.0 + 2.55 * ( long double ) ( i - 1 ) ) 
...
void laguerre_ss_root ( long double *x, int order, long double *dp2, long double *p1, 
...
//  Parameters:
//
//    Input, long double FUNC ( long double X ), the name of the function which
//    evaluates the integrand.  
//
//    Input, long double A, the beginning of the integration interval.
//
//    Input, int ORDER, the order.
//
//    Input, long double XTAB[ORDER], the abscissas.
//
//    Input, long double WEIGHT[ORDER], the weights.
//
//    Output, long double LAGUERRE_SUM, the approximate value of the integral.
//
{
  int i;
  long double result;
...
//
{
  ...
  for ( i = 1; i <= m; i++ )
  {
    ...
    t = ( long double ) ( 4 * i - 1 ) * r8_pi / ( long double ) ( 4 * order + 2 );

    x0 = cos ( t ) * ( 1.0 - ( 1.0 - 1.0 / ( long double ) ( order ) ) 
      / ( long double ) ( 8 * order * order ) );
    ...
    for ( k = 2; k <= order; k++ )
    {
      pkp1 = 2.0 * x0 * pk - pkm1 - ( x0 * pk - pkm1 ) / ( long double ) ( k );
      pkm1 = pk;
      pk = pkp1;
    }

    d1 = ( long double ) ( order ) * ( pkm1 - x0 * pk );
...
//
  ...
  for ( i = 0; i < n; i++ )
  {
    bj[i] = ( long double ) ( ( i + 1 ) * ( i + 1 ) ) 
          / ( long double ) ( 4 * ( i + 1 ) * ( i + 1 ) - 1 );
    bj[i] = sqrt ( bj[i] );
  }
...
//    Input, int EXPON, the exponent.
//
//    Output, long double LEGENDRE_INTEGRAL, the value of the exact integral.
//
{
  long double exact;
//
...
void legendre_recur ( long double *p2, long double *dp2, long double *p1, long double x, int order )
...
//  Parameters:
//
//    Output, long double *P2, the value of P(ORDER)(X).
//
//    Output, long double *DP2, the value of P'(ORDER)(X).
//
//    Output, long double *P1, the value of P(ORDER-1)(X).
//
//    Input, long double X, the point at which polynomials are evaluated.
//
//    Input, int ORDER, the order of the polynomial to be computed.
//
{
  ...
  *dp2 = 1.0;
  ...
  for ( i = 2; i <= order; i++ )
  {
    ...
    *p2 = ( ( long double ) ( 2 * i - 1 ) * x * ( *p1 ) 
          + ( long double ) (   - i + 1 )     * p0 ) 
          / ( long double ) (     i     );

    *dp2 = ( ( long double ) ( 2 * i - 1 ) * ( ( *p1 ) + x * dp1 ) 
           - ( long double ) (     i - 1 ) * dp0 ) 
           / ( long double ) (     i     );
  }
...
void legendre_set ( int n, long double x[], long double w[] )
...
//
  for ( i = 0; i < n; i++ )
  {
    x[i] = cos ( r8_pi * ( long double ) ( i ) / ( long double ) ( n - 1 ) );
  }

  xold = new long double[n];
  p = new long double[n*n];
  ...
  do
  {
    ...
    for ( j = 2; j <= n-1; j++ )
    {
      for ( i = 0; i < n; i++)
      {
        p[i+j*n] = ( ( long double ) ( 2 * j - 1 ) * x[i] * p[i+(j-1)*n]     
                   + ( long double ) (   - j + 1 ) *        p[i+(j-2)*n] ) 
                   / ( long double ) (     j     );
      }
    }
  ...
  for ( i = 0; i < n; i++ )
  {
    w[i] = 2.0 / ( ( long double ) ( ( n - 1 ) * n ) * pow ( p[i+(n-1)*n], 2 ) );
  }
...
void nc_compute_weights ( int n, long double x_min, long double x_max, long double x[], 
  long double w[] )
...
//    Input, int N, the order.
//
//    Input, long double X_MIN, X_MAX, the endpoints of the interval.
//
//    Input, long double X[N], the abscissas.
//
//    Output, long double W[N], the weights.
//
{
...
//  right endpoints.
//
    yvala = d[n-1] / ( long double ) ( n );
    for ( j = n - 2; 0 <= j; j-- )
    {
      yvala = yvala * x_min + d[j] / ( long double ) ( j + 1 );
    }
    yvala = yvala * x_min;

    yvalb = d[n-1] / ( long double ) ( n );
    for ( j = n - 2; 0 <= j; j-- )
    {
      yvalb = yvalb * x_max + d[j] / ( long double ) ( j + 1 );
    }
    yvalb = yvalb * x_max;
...
void ncc_compute ( int n, long double x[], long double w[] )
...
{
  int i;
  long double x_max = 1.0;
  long double x_min = -1.0;
...
void ncc_set ( int order, long double x[], long double w[] )
...
//
{
  ...
  for ( i = 0; i < n; i++ )
  {
    x[i] = ( ( long double ) ( n - i     ) * x_min   
           + ( long double ) (   + i + 1 ) * x_max ) 
           / ( long double ) ( n     + 1 );
  }
...
void nco_set ( int n, long double x[], long double w[] )
...
//
{
  ...
  const long double x_min = -1.0;

  for ( i = 0; i < n; i++ )
  {
    x[i] = ( ( long double ) ( 2 * n - 2 * i - 1 ) * x_min   
           + ( long double ) (         2 * i + 1 ) * x_max ) 
           / ( long double ) ( 2 * n             );
  }
...
void ncoh_set ( int n, long double x[], long double w[] )
...
//
//    Input, long double ALPHA, BETA, the parameters, if required
...
void patterson_set ( int n, long double x[], long double w[] )
...
//
//    Output, long double *X, the argument of the function.
//
//    Output, long double *FX, the value of the function.
//
{
# define N_MAX 11

  long double fx_vec[N_MAX] = { 
...
//  Parameters:
//
//    Input, long double X, the quantity whose absolute value is desired.
//
//    Output, long double R8_ABS, the absolute value of X.
//
{
  long double value;
...
//  Parameters:
//
//    Output, long double R8_EPSILON, the R8 round-off unit.
//
{
  const long double value = 2.220446049250313E-016;
...
//
//    Output, long double R8_FACTORIAL, the factorial of N.
//
{
  int i;
  long double value;
...
//  Purpose:
//
//    R8_FACTORIAL2 computes the long double factorial function.
//
//  Formula:
//
//    FACTORIAL2( N ) = Product ( N * (N-2) * (N-4) * ... * 2 )  (N even)
...
//  Parameters:
//
//    Input, int N, the argument of the long double factorial 
//    function.  If N is less than 1, R8_FACTORIAL2 is returned as 1.0.
//
//    Output, long double R8_FACTORIAL2, the value of N!!.
//
{
  int n_copy;
  long double value;
  ...
  while ( 1 < n_copy )
  {
    value = value * ( long double ) n_copy;
    n_copy = n_copy - 2;
  }
...
//
//    Output, long double R8_GAMMA, the value of the function.
//
{
//
...
//
  ...
  const long double r8_pi = 3.1415926535897932384626434;
  long double res;
  long double sqrtpi = 0.9189385332046727417803297;
  long double sum;
  long double twelve = 12.0;
  long double two = 2.0;
  long double value;
...
//
  if ( y <= zero )
  {
    ...
    if ( res != zero )
    {
      if ( y1 != ( long double ) ( int ) ( y1 * half ) * two )
      {
        parity = true;
      }
...
//  Parameters:
//
//    Input, long double X, the argument of the Gamma function.  X must be positive.
//
//    Output, long double R8_GAMMA_LOG, the logarithm of the Gamma function of X.
...
//
{
  long double c[7] = {
    ...
    -2.777777777777681622553E-03, 
     ...
     5.7083835261E-03 };
  long double corr;
  long double d1 = - 5.772156649015328605195174E-01;
  long double d2 =   4.227843350984671393993777E-01;
  long double d4 =   1.791759469228055000094023;
  long double frtbig = 1.42E+09;
  int i;
  long double p1[8] = {
...
//  Parameters:
//
//    Output, long double R8_HUGE, a "huge" R8 value.
//
{
  long double value;
...
//  Parameters:
//
//    Input, long double A, B, C, X, the arguments of the function.
//    C must not be equal to a nonpositive integer.
//    X < 1.
//
//    Output, long double R8_HYPER_2F1, the value of the function.
//
{
  long double a0;
  long double aa;
  ...
  long double c0;
  long double c1;
  long double el = 0.5772156649015329;
  long double eps;
  long double f0;
  long double f1;
  long double g0;
...
//  Parameters:
//
//    Input, long double X, Y, the quantities to compare.
//
//    Output, long double R8_MAX, the maximum of X and Y.
//
{
  long double value;
...
//  Parameters:
//
//    Input, long double XX, the argument of the function.
//
//    Output, long double R8_PSI, the value of the function.
//
{
  long double aug;
  long double den;
  long double four = 4.0;
  long double fourth = 0.25;
  long double half = 0.5;
  int i;
  int n;
  int nq;
  ...
  long double piov4 = 0.78539816339744830962;
  long double q1[8] = { 
  ...
  long double w;
  long double x;
  long double x01 = 187.0;
  long double x01d = 128.0;
  long double x02 = 6.9464496836234126266E-04;
  long double xinf = 1.70E+38;
  long double xlarge = 2.04E+15;
  long double xmax1 = 3.60E+16;
  long double xmin1 = 5.89E-39;
  long double xsmall = 2.05E-09;
  long double z;
  long double zero = 0.0;
...
//
    else
    {
      ...
      w = w - ( long double ) ( ( int ) ( w ) );
      nq = int ( w * four );
      w = four * ( w - ( long double ) ( nq ) * fourth );
//
...
//  Parameters:
//
//    Input, long double X, the number whose sign is desired.
//
//    Output, long double R8_SIGN, the sign of X.
//
{
  long double value;
...
//
//    Input, long double A1[N], the vector to be copied.
//
//    Input, long double A2[N], the copy of A1.
//
{
  int i;
...
//    Input, int N, the number of entries in the vectors.
//
//    Input, long double A1[N], A2[N], the two vectors to be considered.
//
//    Output, long double R8VEC_DOT_PRODUCT, the dot product of the vectors.
//
{
  int i;
  long double value;
...
//
//    Input, long double A_FIRST, A_LAST, the first and last entries.
//
//    Output, long double A[N], a vector of linearly spaced data.
//
{
  int i;
  ...
  else
  {
    for ( i = 0; i < n; i++ )
    {
      a[i] = ( ( long double ) ( n - 1 - i ) * a_first 
             + ( long double ) (         i ) * a_last ) 
             / ( long double ) ( n - 1     );
    }
  }
  return;
...
//    Input, long double A_FIRST, A_LAST, the first and last entries.
//
//    Output, long double R8VEC_LINSPACE_NEW[N], a vector of linearly spaced data.
//
{
  long double *a;
  int i;
...
//
//    Input, long double A[N], the vector to be printed.
//
...
void r8vec_reverse ( int n, long double a[] )
...
//  Discussion:
//
//    An R8VEC is a vector of long double precision values.
//
//    Input:
//
//      N = 5,
...
//
//    Input/output, long double A[N], the array to be reversed.
//
{
  int i;
  long double temp;
...
//
  for ( i = 0; i < n; i++ )
  {
    x[i] = - cos ( 2.0 * r8_pi * ( long double ) (         i ) 
                               / ( long double ) ( 2 * n - 1 ) );
  }
  xold = new long double[n];
  ...
  do
  {
    ...
    for ( i = 1; i < n; i++ )
    {
      x[i] = xold[i] - ( ( 1.0 - xold[i] ) / ( long double ) ( n ) ) 
  ...
  w[0] = 2.0 / ( long double ) ( n * n );
  for ( i = 1; i < n; i++ )
  {
    w[i] = ( 1.0 - x[i] ) / pow ( ( long double ) ( n ) * p[i+(n-1)*n], 2 );
  }
  delete [] xold;
  delete [] p;
...
//    Input, int NT, the number of knots.
//
//    Input, long double T[NT], the original knots.
//
//    Input, int MLT[NT], the multiplicity of the knots.
//
//    Input, long double WTS[NWTS], the weights.
//
...
//
//    Output, long double SWTS[NWTS], the scaled weights.
//
//    Output, long double ST[NT], the scaled knots.
//
//    Input, int KIND, the rule.
//    1, Legendre,             (a,b)       1.0
//    2, Chebyshev Type 1,     (a,b)       ((b-x)*(x-a))^(-0.5)
...
//    Input, long double BETA, the value of Beta, if needed.
//
//    Input, long double A, B, the interval endpoints.
//
{
  long double al;
  long double be;
...
//    Input, int NT, the number of knots.
//
//    Input, long double AJ[NT], the diagonal of the Jacobi matrix.
//
//    Input/output, long double BJ[NT], the subdiagonal of the Jacobi 
//    matrix, in entries 1 through NT-1.  On output, BJ has been overwritten.
//
//    Input, long double ZEMU, the zero-th moment of the weight function.
//
//    Output, long double T[NT], the knots.
//
//    Output, long double WTS[NT], the weights.
